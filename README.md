...
1. 📬 前端或 API 提交配置
2. 📤 Config Worker 初始化任务（写入 DO）
3. 📦 推送任务至 DownloadQueue
4. 📥 Download Worker 下载图片至 R2
5. 📤 推送任务至 MetadataQueue
6. 📄 Metadata Worker 提取信息并写入 D1
7. 📤 推送任务至 AIQueue
8. 🤖 AI Worker 调用分析并写入 D1/Vectorize
9. ✅ 所有状态在 TaskCoordinator DO 中持续更新
...
| 后端 | Cloudflare Workers (11 个) |
| 存储 | R2, D1, Vectorize |
| 消息 | Cloudflare Queues + DLQs |
| 状态协调 | Durable Objects |
| 日志与追踪 | Axiom + TraceID + Logpush |
| 安全认证 | JWT / HMAC + Cloudflare Secrets |
| 基础设施 | Terraform（计划迁移至 GitLab Remote State） |
| CI/CD | GitLab Pipelines（in progress） |
| 工具链 | pnpm, Turborepo, Wrangler, Vitest |
